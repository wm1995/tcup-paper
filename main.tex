...
$t$-distribution vs when analysed using a normal distribution, along with an
estimate of this ratio using the Fisher information of a $t$-distribution
likelihood (see Appendix \ref{sec:fisher}). While there can be a $\sim20\%$ error
in the Cauchy regime ($\nu \approx 1$), this could be considered an acceptable
trade-off to reduce bias in cases of model mis-specification. Under a flexible
model that infers $\nu$ (such as the one presented here), we would expect to the
model to infer a value for $\nu$ that approaches a normal distribution,
mitigating this issue of increased error.
...
repeated trials alongside the Cram\'er-Rao bound in Figure
\ref{fig:formalism.posterior-sd}; here, we derive the formula for this bound.
...
$t$-distribution, is
\begin{equation}
    \ell (\mu; \nu, \{x_i\}) = - \sum_i \frac{\nu + 1}{2} \log (1 + \frac{(x_i - \mu)^2}{\nu}) + C,
\end{equation}
where $C$ is a constant. Thus, the Fisher information is
\begin{equation}
    \mathcal F(\mu; \nu, \{x_i\}) = (\nu + 1) \sum_i \frac{\nu - (x_i - \mu)^2}{\nu + (x_i - \mu)^2}.
\end{equation}
We can then evaluate the Cram\'er-Rao bound at the true mean, marginalising over
the particular dataset $\{x_i\}$, to derive the variance in the estimate at $\mu
= 0$,
\begin{equation}
...
This bound was found to agree with the results from datasets generated with $N =
10^5$, and is used to derive the posterior standard deviation ratio plotted in
