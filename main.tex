...
bias in cases of model mis-specification.  This uncertainty trade-off is
mitigated by a flexible model that infers $\nu$ (such as the one presented
here), as we would expect the model to infer a value for $\nu$ that approaches a
normal distribution. We argue that the increased numerical difficulty of
treating $\nu$ as a parameter in this way is worth reducing the increased
error that is associated with fixed $\nu$ analyses.
