...
reanalysis of \citet{Riess:2011} by \citet{Efstathiou:2014}). A more principled
...
before concluding remarks in Section \ref{sec:conclusion}.
...
The second quantity we defined is an ``outlier fraction'' $\omega$, which
defines the fraction of points expected to lie more than $3\sigma$ from the
...
model is designed to be robust to model misspecification; the only key element
...
simulation-based calibration tests indicate that the rank statistic is
...
\sigma_{68}, \nu\} = \{3, 2, 0.1, 3\}$; the full model used to generate the data
is specified in Appendix \ref{sec:data-models.t}. The chosen value of $\nu = 3$
corresponds to an outlier fraction of $\omega(\nu = 3) \approx 5.8 \%$.  As we
can see in Figure \ref{fig:results.t.corner}, we recover the values of the
