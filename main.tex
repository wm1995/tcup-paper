...
\end{equation}
where $I^{-1}$ is the inverse regularised incomplete beta function.
...
distribution, i.e., as $\nu \rightarrow \infty$.  Under this definition approximately 1 in 370 data-points would be outliers for data that follows a
normal distribution; for Cauchy-distributed data (i.e. $\nu = 1$),
every fifth data-point to be an outlier.
...
{\color{green} Should this be theta-x rather than theta-f?}

{\color{green} Say something about options for extending to non-linear models.}
...
{\color{green} Please proof read this section!}

One potential disadvantage of adopting a heavy-tailed likelihood in the statistical model is that the resultant inferences would, in general, be less constraining than under the assumption of a normal model.  If there are no outliers the implication would be that the robust approach is inferior. 
This effect would be appreciable for the smallest datasets -- the posteriors would have heavier tails than under a normal model -- but the lack of means meaningful constraints are unlikely irrespective of the adopted model. For larger datasets, of more than a few tens of points, these tails are effectively multiplied out, typically leaving just a single region of high probability.  Moreover, for the regression problem considered here, the posterior in the regression parameters satisfies the requirements for asymptotic normality \citep[e.g.][]{Ghosh_etal:2006}, so the core of the posterior is Gaussian in form, just as would be the case under a normal model.  (There is not even a significant numerical cost as the posterior has to be evaluated by sampling anyway.)
...
the above structure, but uses normal distributions at each stage --- we refere to this as $n$-cup.
...
{\color{green} But isn't P-int normal so there aren't two heavy-tailed distributions in the model?}
...
\sigma_{68}, \nu\} = \{3, 2, 0.1, 3\}$;
{\color{green} Lots of cases like this where they aren't really sets, as they're ordered; so more corrected to put () brackets around these.}
the full model used to generate the data
is specified in Appendix \ref{sec:data-models.t}. The chosen value of $\nu = 3$
corresponds to an outlier fraction of $\omega(\nu = 3) \approx 5.8 \%$.  As shown in Figure \ref{fig:results.t.corner}, we recover the values of the
...
the intrinsic scatter $\sigma_{68}$. For the nusiance paramater $\nu$, the
...
$\mathcal P_{\rm int}$ to be a log-normal distribution.
...
0.05$\raisebox{0.5ex}{\tiny$^{+0.04}_{-0.01}$}). This showcases that analysing
...
$M_{\text{BH}}$, \textsc{Civ} emission line widtha $\Delta V$, and continuum
luminosities $\lambda L_{\lambda}$.
