...
if __name__ == "__main__":
    ...
    mcmc_regex = re.compile(".*/\d{1,3}\.nc")
    mcmc_files = filter(lambda x: mcmc_regex.match(str(x)), results_path.glob("*.nc"))
    ...
    for mcmc_file in tqdm(mcmc_files):
        ...
        for group in results:
            if results[group] is None:
                ...
                results[group] = xr.concat([results[group], samples], dim="sample")

    for group, samples in results.items():
        samples = samples.reset_index(["sample", "chain", "draw"], drop=True)
        samples.to_netcdf(results_path / "results.nc", group=group, engine="h5netcdf", mode="a")
