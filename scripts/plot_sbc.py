...
if __name__ == "__main__":
    ...
    var_names = ["alpha_scaled", "beta_scaled.0", "beta_scaled.1", "sigma_scaled", "nu"]
    ...
    for filename in glob(results_path + "*.nc"):
        ...
        data_filename = data_path + filename.split("/")[-1].split(".")[0] + ".json"
        try:
        ...
        min_ess = az.ess(mcmc, var_names=[var_name.split(".")[0] for var_name in var_names]).to_array().min()
        ...
        for var_name, curr_bins in zip(var_names, bins):
            if "." in var_name:
                ...
                var_dim = int(var_dim)
                samples = mcmc.posterior[raw_var_name].values[:, :, var_dim].flatten()
            ...
            ranks[var_name].append((dataset_value, rank))
    ...
    print(f"{N=}, {B=}, {N / B=:.2f}")
    print(f"{confidence_level * 100:.0f}% confidence interval: [{lower}, {upper}]")
