...
def test_outlier_dataset(dataset_params, outlier_sigma):
    ...
    assert np.isclose(
        outlier_logpdf,
        -np.log(np.pi * 2) / 2 - np.log(dataset.sigma_68) - outlier_sigma**2 / 2
    )
...
def test_gaussian_mix_dataset(dataset_params):
    ...
    assert sps.kstest(z[dataset.outlier_mask], sps.norm(scale = 10).cdf).pvalue > THRESHOLD
